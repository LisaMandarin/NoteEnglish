...
**Phase 1/2/3 全數完成並經使用者實機驗證（2026-07-05）**：

1. ✅ yet-compound 例句重測：「（S）」後綴已消失，兩個子句 badge 乾淨顯示「SVO 主要子句」「SVC 主要子句」，頂層「合句」badge 正確、無殘留單一 pattern。
2. ✅ 手機寬度實機截圖：文字自然換行，「主要子句」方塊未溢出畫面，badge 顯示正常。

目前沒有已排入計畫的下一步。已知殘留（非限定關係子句/古典引文/since-after/重後位修飾的 degraded serve，見上方清單）維持現狀，品質可接受，暫不列入任務。
